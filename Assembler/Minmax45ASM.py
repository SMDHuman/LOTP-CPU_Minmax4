...
def generate_bytes(tokens: list[Token]) -> bytearray:
  ...
  while(tokens.current() != None):
    ...
    #---------------------------------------
      ...
      header = tokens.next()
      # Check if the word is a constant decleration or an instruction
      ...
      #...
        ...
        values = []
        # Argument 1
        if(header.word.upper() not in ["HLT", "JMP"]):
        ...
        # Argument 2
        if(header.word.upper() not in ["PSH", "POP", "HLT"]):
          if(header.word.upper() == "IN"):
